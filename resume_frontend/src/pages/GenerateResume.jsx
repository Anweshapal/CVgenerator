...
//   // Initial form data with empty arrays containing one empty object to show fields on start
//   const initialData = {
//     personalInformation: {
...
//     try {
//       setLoading(true);
//       const responseData = await generateResume({ userDescription: description });

// if (responseData.error) {
//   toast.error("Error Generating Resume: " + responseData.error);
//   setLoading(false);
//   return;
// }
...
const GenerateResume = () => {
  ...
  const renderFieldArray = (fields, label, name, keys) => {
    return (
      <div className="form-control w-full mb-4" key={name}>
        ...
        >
          <FaPlusCircle className="w-5 h-5 text-base-content" /> Add {label}
        </button>
      </div>
    );
  };
  ...
  function showResume() {
    return (
      <div>
        {/* Pass full data, not just projects */}
        <Resume data={data} />
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-base-300 text-base-content">
      {showPromptInput && ShowInputField()}
      {showFormUI && showFormFunction()}
      {showResumeUI && showResume()}
    </div>
